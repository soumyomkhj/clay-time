...
#the final tune
def play_tone(samples,count,duration):
	...
	for x in range(count):
		...
		if x!=count-1:
			next_shape=samples[x+1].x1-samples[x].x1
			time.sleep(next_shape*duration*.9)
		...
		if ((index%10)==0):
			...
			for shape_no in range(count):
				...
				for x_pos in range(obj_pos[shape_no][0],obj_pos[shape_no][0]+obj_pos[shape_no][2]):
					...
					frame_copy = cv2.line(frame_copy, (x_pos,top), (x_pos,bottom), (255, 255, 0), 1)
					cv2.imshow("Scanning", frame_copy)
					# break	
					...
					# print(shape_no)
					sample=np.append(sample,createTone(volume/50,f,duration))
					sample=np.array(sample)
				
				#list of all the 
				samples.append(clay_shape(sample,obj_pos[shape_no][0]))
			cv2.imshow("Scanning", frame)
			play_tone(samples,count,duration)
